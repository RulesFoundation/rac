...
class RustGenerator:
    ...
    def __init__(self, ir: IR, module_name: str):
        ...
        self.indent = 0
        # Track entity vars for each entity
    ...
    def _gen_input_struct(self, entity) -> list[str]:
        """Generate input struct for an entity."""
        lines = [
            "#[derive(Debug, Clone)]",
    ...
    def _gen_output_struct(self, entity_name: str, var_paths: list[str]) -> list[str]:
        """Generate output struct for computed entity vars."""
        lines = [
    ...
    def _gen_scalars_struct(self) -> list[str]:
        """Generate struct holding scalar parameters."""
        lines = [
            "#[derive(Debug, Clone)]",
            "pub struct Scalars {",
        ]
    ...
    def _gen_compute_entity(self, entity_name: str) -> list[str]:
        """Generate entity computation function."""
        ...
        # Build list of computed var names in order
        computed_names = []
        for path in var_paths:
            safe_name = self._rust_ident(path.replace("/", "_"))
            var = self.ir.variables[path]
            expr_code = self._gen_expr(var.expr, entity_var="input", scalars_var="scalars", computed=computed_names.copy())
            lines.append(f"        let {safe_name} = {expr_code};")
    ...
    def _gen_expr(self, expr: ast.Expr, entity_var: str | None = None,
                  scalars_var: str | None = None, computed: list[tuple[str, str]] | None = None) -> str:
        ...
        match expr:
            ...
            case ast.Var(path=path):
                # Check if it's a previously computed entity var
                if path in computed_lookup:
                    return computed_lookup[path]
                # Check if it's a scalar (has /)
                if "/" in path:
